...
class Solution {
   private:
    ...
    vector<char> findOptimalSchedule(vector<char>& tasks, int cooldownPeriod) {
        int len = tasks.size();
        if (len == 0) {
            if (debug) {
                printf("returning 0\n");
   ...
   public:
    ...
    int leastInterval(vector<char>& tasks, int n) {
        vector<char> schedule = findOptimalSchedule(tasks, n);
...
class SolutionTest {
   public:
    void testFindOptimalSchedule() {
        ...
        coolDownPeriod = 5;
        scheduleOutExpected = {};
        scheduleOutComputed = soln.findOptimalSchedule(tasks, coolDownPeriod);
...
int main() {
    ...
    solnTest.testFindOptimalSchedule();
