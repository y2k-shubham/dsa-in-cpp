...
// AlgorithmsMadeEasy: https://www.youtube.com/watch?v=wAxhnUhXvHE
 ...
 * @date 2021-10-02
 * 
 * method 1 mostly works but fails in following test-case (might fail in more cases)
 ...
 * computed-answer: 586
 * .
 * furthermore, method 1 is really complex & quite long
 */
...
class Solution {
   private:
    ...
    int maximizeReach2(
        ...
        fillVec[0] = '-';  // we start with 0th index: it is always visited
        int nextStep = 1;
        int crrStep = 0;
        while (nextStep < len) {
            if (debug) {
            ...
            crrStep = nextStep - 1;
            int jumpSize = heights[nextStep] - heights[crrStep];
            bool advance = false;

            if (jumpSize <= 0) {
                // no bricks / ladders needed
                fillVec[crrStep] = '-';
                advance = true;
            } else if (remLadders > 0) {
                // use a ladder
                fillVec[crrStep] = 'L';
                laddersQue.push({jumpSize, crrStep});
                remLadders--;
                advance = true;
            } else {
                // try to see if previously used ladder can be put here
                bool isLaddersQEmpty = laddersQue.empty();
                pair<int, int> ladderMinJumpTuple = {-1, -1};
                if (!isLaddersQEmpty) {
                    ladderMinJumpTuple = laddersQue.top();
                }

                int ladderMinJumpSize = ladderMinJumpTuple.first;
                int ladderMinJumpIdx = ladderMinJumpTuple.second;

                bool canOptimize = !isLaddersQEmpty && (jumpSize > ladderMinJumpSize) && (remBricks >= ladderMinJumpSize);
                if (canOptimize) {
                    // if ladder's min jump posn can be substituted with remaining bricks,
                    // then we can put that ladder at current posn instead (and end up using lesser bricks)
                    fillVec[ladderMinJumpIdx] = 'B';
                    remBricks -= ladderMinJumpSize;

                    // update the pQue holding ladder posns
                    laddersQue.pop();
                    fillVec[crrStep] = 'L';
                    laddersQue.push({jumpSize, crrStep});

                    advance = true;
                } else if (remBricks >= jumpSize) {
                    // use bricks
                    fillVec[crrStep] = 'B';
                    remBricks -= jumpSize;
                    advance = true;
   ...
   public:
    ...
    int furthestBuilding(vector<int>& heights, int bricks, int ladders) {
        ...
        // return maximizeReach1(heights, fillVec, bricks, ladders);
        return maximizeReach2(heights, fillVec, bricks, ladders);
    }
};

class SolutionTest {
   public:
    void testMaximizeReach1() {
        ...
        totalLadders = 2;
        reachOutExpected = 0;
        reachOutComputed = soln.maximizeReach1(heights, fillVecIn, totalBricks, totalLadders);
    ...
    void testMaximizeReach2() {
        ...
        totalLadders = 2;
        reachOutExpected = 0;
        reachOutComputed = soln.maximizeReach2(heights, fillVecIn, totalBricks, totalLadders);
        assert(reachOutExpected == reachOutComputed);
        fillVecOutExpected = {};
        assert(fillVecOutExpected == fillVecIn);
        ...
        fillVecIn = soln.createFillVec(heights);
        totalBricks = 33671263;
        totalLadders = 108;
        reachOutExpected = 589;
        reachOutComputed = soln.maximizeReach2(heights, fillVecIn, totalBricks, totalLadders);
        // fillVecOutExpected = {'-', 'L', '-', 'B', 'B', '.', '.'};
        // soln.showFillVec(heights, fillVecIn);
        // assert(fillVecOutExpected == fillVecIn);
...
int main() {
    ...
    solnTest.testMaximizeReach1();
    solnTest.testMaximizeReach2();
    return 0;
}
