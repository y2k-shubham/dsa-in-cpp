...
void findShortPath(vector <list <pair <int, int> > >, int, queue <int>, vector <int> &, vector <int> &, int);
int add(int, int);
...
int main() {
	...
	findShortPath(adjList, V, topoOrder, shortPath, parent, src);
	showList(parent, V, "parent of various vertices", "par");
	showList(shortPath, V, "shortest path to various nodes", "path");
...
void showList(vector <int> shortPath, int V, const char * msg, const char * name) {
	printf("\nThe %s are:-\n", msg);
...
void findShortPath(vector <list <pair <int, int> > > adjList, int V, queue <int> topoOrder, vector <int> &shortPath, vector <int> &par, int src) {
	...
	while (!topoOrder.empty()) {
		...
		topoOrder.pop();
//		printf("\nDequeued\tVert %d\tPath = %d\tPar = %d\n", crrVert, shortPath[crrVert], par[crrVert]);

		for (list <pair <int, int> >::iterator it = adjList[crrVert].begin(); it != adjList[crrVert].end(); it++) {
			if (shortPath[it -> first] > add(shortPath[crrVert], it -> second)) {
//				int origPath = shortPath[it -> first];
//				int origPar = par[it -> first];
				shortPath[it -> first] = add(shortPath[crrVert], it -> second);
				par[it -> first] = crrVert;

//				if (origPath != INT_MAX) {
//					printf("Updated Vertex %d\tfrom\tPath %d\tPar %d\tto\tPath %d\tPar %d\n", it -> first, origPath, origPar, shortPath[it -> first], par[it -> first]);
//				} else {
//					printf("Updated Vertex %d\tfrom\tPath INF\tPar %d\tto\tPath %d\tPar %d\n", it -> first, origPar, shortPath[it -> first], par[it -> first]);
//				}
			}
		}
	}
