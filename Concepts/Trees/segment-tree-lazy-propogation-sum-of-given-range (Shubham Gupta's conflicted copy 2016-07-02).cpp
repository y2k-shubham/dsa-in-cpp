...
void lazyprop(int tree[MAX], int lazyTree[MAX], int lo, int hi, int pos, int uLo, int uHi, int delta)
{
	if(lazytree[pos] != 0)
	{
		tree[pos] +=(hi - lo + 1)* lazytree[pos];
		if(hi != lo)
		{
...
int sumQuery(int tree[MAX], int lazyTree[MAX], int lo, int hi, int pos, int qLo, int qHi) {
	if (hi < qLo || qHi < lo) {
		...
		lazyTree[lC(pos)] += lazyTree[pos];s
		lazyTree[rC(pos)] += lazyTree[pos];
		lazyTree[pos] = 0;
