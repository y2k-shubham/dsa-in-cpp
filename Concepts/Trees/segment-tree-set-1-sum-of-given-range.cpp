...
int sumQuery(int [MAX], int, int, int, int, int);
		...
		switch (choice) {
			case 1:
				...
				if (0 <= qLo && qHi <= (arrLen - 1)) {
					printf("Sum in range arr[%d] - arr[%d] is %d\n", qLo, qHi, sumQuery(tree, 0, arrLen - 1, 0, qLo, qHi));
				} else {
...
int sumQuery(int tree[MAX], int lo, int hi, int pos, int qLo, int qHi) {
	if (qHi < lo || hi < qLo) {
		// no overlap
		return 0;
		...
		// partial overlap
		int mid = (lo + hi) / 2;
		return sumQuery(tree, lo, mid, lChild(pos), qLo, qHi) + sumQuery(tree, mid + 1, hi, rChild(pos), qLo, qHi);
	}
}
...
void updateTree(int tree[MAX], int lo, int hi, int pos, int uLo, int uHi, int delta) {
	if (uHi < lo || hi < uLo) {
		...
		// just 1 element, update made to actual element of array
		tree[pos] += delta;
	} else {
		int mid = (lo + hi) / 2;
